...
/** Модель для источника данных ZmlApp.source.CategoriesComboData */
Ext.define('ZmlApp.model.CategoriesComboData', {
  extend: 'Zml.data.Model',
  idProperty: 'Id',
  fields: [
    { name: 'Id', type: 'int' },
    { name: 'Name', type: 'string' },
    { name: 'GroupsCount', type: 'int' }
  ]
});

/** Хранилище данных для списка категорий затрат. */
Ext.define('ZmlApp.source.CategoriesComboData', {
  extend: 'Zml.data.RestStore',
  model: 'ZmlApp.model.CategoriesComboData',
  storeId: 'CategoriesComboData',
  ObjectType: 'source',
  KEY: 'Id',
  autoLoad: true,
  autoDestroy: false,
  ...
  proxy: { type: 'RESTProxy', idParam: 'Id', url: '/', path: 'Expenses/Exp/Categories' }
});
...
/** Модель для источника данных ZmlApp.source.GroupsComboData */
Ext.define('ZmlApp.model.GroupsComboData', {
  extend: 'Zml.data.Model',
  idProperty: 'Id',
  fields: [
    { name: 'Id', type: 'int' },
...
/** Хранилище данных для списка групп, входящих в категорию затрат. */
Ext.define('ZmlApp.source.GroupsComboData', {
  extend: 'Zml.data.RestStore',
  model: 'ZmlApp.model.GroupsComboData',
  storeId: 'GroupsComboData',
  ObjectType: 'source',
  KEY: 'Id',
  autoLoad: true,
  autoDestroy: false,
  ...
  paramLinkers: [
    new Zml.Param({ scopeId: 'CatCombo', scopeType: 'combo', field: 'valueField', as: 'catid' })
  ],
  proxy: { type: 'RESTProxy', idParam: 'Id', url: '/', path: 'Expenses/Exp/Groups' }
});
...
/** Модель для источника данных ZmlApp.source.GetExpRules */
Ext.define('ZmlApp.model.GetExpRules', {
  extend: 'Zml.data.Model',
  idProperty: 'Id',
  fields: [
    { name: 'Id', type: 'int' },
...
/** Хранилище данных для правил авторазнесения затрат в определенную группу. */
Ext.define('ZmlApp.source.GetExpRules', {
  extend: 'Zml.data.RestStore',
  model: 'ZmlApp.model.GetExpRules',
  storeId: 'GetExpRules',
  ObjectType: 'source',
  KEY: 'Id',
  autoLoad: false,
  ...
  Columns: {
    defaults: { filterable: true, sortable: true },
    items: [{ text: 'Имя', dataIndex: 'Name', flex: 1 }]
  },
  paramLinkers: [
    new Zml.Param({ scopeId: 'GroupsCombo', scopeType: 'combo', field: 'valueField', as: 'grpid' })
  ],
  proxy: { type: 'RESTProxy', idParam: 'Id', url: '/', path: 'Expenses/api/ExpRules' }
});
...
/** Модель для источника данных ZmlApp.source.GetIncludePatterns */
Ext.define('ZmlApp.model.GetIncludePatterns', {
  extend: 'Zml.data.Model',
  idProperty: 'Id',
  fields: [
    { name: 'Id', type: 'int' },
...
/** Хранилище данных для паттернов соответствия, ассоциированных с определенным правилом. */
Ext.define('ZmlApp.source.GetIncludePatterns', {
  extend: 'Zml.data.RestStore',
  model: 'ZmlApp.model.GetIncludePatterns',
  storeId: 'GetIncludePatterns',
  ObjectType: 'source',
  KEY: 'Id',
  autoLoad: false,
  autoDestroy: false,
  ...
  Columns: {
    defaults: { filterable: true, sortable: true },
    items: [{ text: 'Паттерн', dataIndex: 'Pattern', flex: 1, editor: { xtype: 'TextField' } }]
  },
  paramLinkers: [
    new Zml.Param({ scopeId: 'RulesGrid', scopeType: 'grid', field: 'Id', as: 'ruleid' })
  ],
  proxy: { type: 'RESTProxy', idParam: 'Id', url: '/', path: 'Expenses/api/RuleMatches' },
  OnAfterWrite: function ()
  {
    ZmlApp.OnAfterMatchesSaved();
  }
});
...
/** Модель для источника данных ZmlApp.source.GetExcludePatterns */
Ext.define('ZmlApp.model.GetExcludePatterns', {
  extend: 'Zml.data.Model',
  idProperty: 'Id',
  fields: [
    { name: 'Id', type: 'int' },
...
/** Хранилище данных для паттернов несоответствия, ассоциированных с определенным правилом. */
Ext.define('ZmlApp.source.GetExcludePatterns', {
  extend: 'Zml.data.RestStore',
  model: 'ZmlApp.model.GetExcludePatterns',
  storeId: 'GetExcludePatterns',
  ObjectType: 'source',
  KEY: 'Id',
  autoLoad: false,
  autoDestroy: false,
  ...
  proxy: { type: 'RESTProxy', idParam: 'Id', url: '/', path: 'Expenses/api/RuleNotMatches' },
  OnAfterWrite: function ()
  {
    ZmlApp.OnAfterMatchesSaved();
  }
  ...
  items: [
    {
      ...
      titleCollapse: false,
      split: false,
      plugins: [{ ptype: 'ColorManager', titleColor: 'white', titleBackColor: 'blue', bodyBackColor: 'white' }],
      items: [
        {
          xtype: 'form',
          itemId: 'FiltersForm',
          ...
          layout: 'anchor',
          columnWidth: 1,
          plugins: [{ ptype: 'ColorManager', titleColor: 'black', titleBackColor: 'cleargrey', bodyBackColor: 'white' }],
          items: [
            {
              xtype: 'ComboBox',
              fieldLabel: 'Категории затрат',
              itemId: 'CatCombo',
              displayField: 'Name',
              valueField: 'Id',
              store: 'CategoriesComboData',
              editable: false,
              forceSelection: true,
              anchor: '100%',
              labelWidth: 150,
              TEMPLATE: ['<ul class="x-list-plain">', '<tpl for=".">', '<li role="option" class="x-boundlist-item combo-item">', '<div class="first-line">', '<p class="left-side">{Name}</p>', '<p class="center-side"> ₽</p>', '<p class="right-side">{GroupsCount} зап.</p>', '</div>', '</li>', '</tpl>', '</ul>']
            },
            {
              xtype: 'ComboBox',
              fieldLabel: 'Группы затрат',
              itemId: 'GroupsCombo',
              displayField: 'Name',
              valueField: 'Id',
              store: 'GroupsComboData',
              editable: false,
              forceSelection: true,
              anchor: '100%',
              labelWidth: 150,
              TEMPLATE: ['<ul class="x-list-plain">', '<tpl for=".">', '<li role="option" class="x-boundlist-item combo-item">', '<div class="first-line">', '<p class="left-side">{Name}</p>', '<p class="center-side"> ₽</p>', '<p class="right-side"> зап.</p>', '</div>', '</li>', '</tpl>', '</ul>']
            }
          ]
        },
        { xtype: 'splitter', collapseTarget: 'next', size: 6, overCls: 'x-splitter-over' },
        {
          xtype: 'CommonGrid',
          itemId: 'RulesGrid',
          title: 'ПРАВИЛА РАЗНЕСЕНИЯ ЗАТРАТ',
          flex: 1,
          selModel: { selType: 'rowmodel', allowDeselect: true, mode: 'SINGLE' },
          store: 'GetExpRules',
          viewConfig: { },
          features: [{ ftype: 'filters', encode: true }],
          plugins: [{ ptype: 'ColorManager', titleColor: 'white', titleBackColor: 'darkgreen', bodyBackColor: 'white' }, { ptype: 'GridEditor' }, { ptype: 'rowediting', pluginId: 'cedit', clicksToEdit: 2 }],
          columns: {
            defaults: { filterable: true, sortable: true },
            items: [{ text: 'Имя', dataIndex: 'Name', flex: 1 }]
          },
    ...
    { xtype: 'splitter', collapseTarget: 'next', size: 10, overCls: 'x-splitter-over' },
    {
      ...
      items: [
        {
          xtype: 'CommonGrid',
          itemId: 'RuleMatchGrid',
          title: 'ВКЛЮЧАТЬ',
          selModel: { selType: 'rowmodel', allowDeselect: true, mode: 'SINGLE' },
          store: 'GetIncludePatterns',
          viewConfig: { },
          features: [{ ftype: 'filters', encode: true }],
        ...
        {
          xtype: 'CommonGrid',
          itemId: 'RuleNotMatchGrid',
          title: 'ИСКЛЮЧАТЬ',
          selModel: { selType: 'rowmodel', allowDeselect: true, mode: 'SINGLE' },
          store: 'GetExcludePatterns',
          viewConfig: { },
          features: [{ ftype: 'filters', encode: true }],
          plugins: [{ ptype: 'ColorManager', titleColor: 'white', titleBackColor: 'blue', bodyBackColor: 'white' }, { ptype: 'GridEditor' }, { ptype: 'rowediting', pluginId: 'cedit', clicksToEdit: 2 }],
          columns: {
...
Ext.define('ZmlApp.controller.Main', {
  ...
  models: ['ZmlApp.model.CategoriesComboData', 'ZmlApp.model.GroupsComboData', 'ZmlApp.model.GetExpRules', 'ZmlApp.model.GetIncludePatterns', 'ZmlApp.model.GetExcludePatterns'],
  init: function ()
  {
    this.initStores();
    this.initView();
  ...
  initStores: function ()
  {
    Ext.create('ZmlApp.source.CategoriesComboData', { storeId: 'CategoriesComboData' });
    Ext.create('ZmlApp.source.GroupsComboData', { storeId: 'GroupsComboData' });
    Ext.create('ZmlApp.source.GetExpRules', { storeId: 'GetExpRules' });
    Ext.create('ZmlApp.source.GetIncludePatterns', { storeId: 'GetIncludePatterns' });
    Ext.create('ZmlApp.source.GetExcludePatterns', { storeId: 'GetExcludePatterns' });
  }
});
...
Ext.application({
  ...
  launch: function ()
  {
    ...
    ZmlApp.LOC = {
      ...
      Combo: {
        CatCombo: ZmlApp.VIEW.query('combobox[itemId=CatCombo]')[0],
        GroupsCombo: ZmlApp.VIEW.query('combobox[itemId=GroupsCombo]')[0]
      },
      List: {
      },
      Grid: {
        RulesGrid: ZmlApp.VIEW.query('CommonGrid[itemId=RulesGrid]')[0],
        RuleMatchGrid: ZmlApp.VIEW.query('CommonGrid[itemId=RuleMatchGrid]')[0],
        RuleNotMatchGrid: ZmlApp.VIEW.query('CommonGrid[itemId=RuleNotMatchGrid]')[0]
      },
      Tree: {
      },
      Form: {
        FiltersForm: ZmlApp.VIEW.query('form[itemId=FiltersForm]')[0]
      },
      Button: {
      },
      Panel: {
    ...
    ZmlApp.SRC = {
      CategoriesComboData: Ext.getStore('CategoriesComboData'),
      GroupsComboData: Ext.getStore('GroupsComboData'),
      GetExpRules: Ext.getStore('GetExpRules'),
      GetIncludePatterns: Ext.getStore('GetIncludePatterns'),
      GetExcludePatterns: Ext.getStore('GetExcludePatterns')
    };
    Ext.StoreManager.AutoloadSources();
    ZmlApp.SaveNewRule = function ()
    {
      Ext.StoreManager.SAVENEWRECORD('GetExpRules', 
      [
        new Zml.Param({ as: 'Id', value: '0' }),
        new Zml.Param({ as: 'Name', value: '' }),
        new Zml.Param({ scopeId: 'GroupsCombo', scopeType: 'combo', field: 'valueField', as: 'GrpId' })
      ]);
    };
    ZmlApp.SaveNewRuleMatch = function ()
    {
      Ext.StoreManager.SAVENEWRECORD('GetIncludePatterns', 
      [
        new Zml.Param({ as: 'Id', value: '0' }),
        new Zml.Param({ scopeId: 'RulesGrid', scopeType: 'grid', field: 'Id', as: 'RuleId' }),
        new Zml.Param({ as: 'Pattern', value: '' })
      ]);
    };
    ZmlApp.SaveNewRuleNotMatch = function ()
    {
      Ext.StoreManager.SAVENEWRECORD('GetExcludePatterns', 
      [
        new Zml.Param({ as: 'Id', value: '0' }),
        new Zml.Param({ scopeId: 'RulesGrid', scopeType: 'grid', field: 'Id', as: 'RuleId' }),
        new Zml.Param({ as: 'Pattern', value: '' })
    ...
    ZmlApp.OnAfterMatchesSaved = function ()
    {
      Ext.getStore('GetExpRules').load();
    };
  }
});
